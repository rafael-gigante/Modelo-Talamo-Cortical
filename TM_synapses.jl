...
function TMsynE_inst_D(r, x, Is, sp_event)
    ...
    taud = [138, 671, 329]
    U = [0.09, 0.5, 0.29]
    A = [0, 1, 0] # Only the inhibitory synapse (D) contributes
    
    taus = 3
    
    r = r .+ dt .* ((-r ./ tauf ) .+ (U .* (1 .- r) .* sp_event))
    x_old = x
    x = x .+ dt .* (((1 .- x) ./ taud) .- (r .* x_old .* sp_event))
    Is = Is .+ dt .* ((-Is ./ taus) .+ A .* (r .* x_old .* sp_event))
...
function TMsynE_inst_F(r, x, Is, sp_event)
    ...
    taud = [138, 671, 329]
    U = [0.09, 0.5, 0.29]
    A = [1, 0, 0]  # Only the facilitating synapse (F) contributes
