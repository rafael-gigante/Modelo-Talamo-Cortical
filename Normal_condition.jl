...
W_EEs = (-1e1 / fac) .* r_s # S to S
W_EEm = (-1e1 / fac) .* r_m # M to M
W_EEd = (-1e1 / fac) .* r_d # D to D
W_IIins = (-5e2 / fac) .* r_ins # INs to INs
W_IIret = (-5e1 / fac) .* r_ret # Reticular to Reticular
W_EErel = (0 / fac) .* r_rel # Relay to Relay

# Forças de acoplamento entre estruturas
# S
W_EEsm = (1e1 / fac) .* r_s # M to S
W_EEsd = (5e2 / fac) .* r_s # D to S
W_EIsINs = (-5e2 / fac) .* r_s # INs to S
W_EIsRet = (0 / fac) .* r_s # Reticular to S
W_EEsRel = (0 / fac) .* r_s # Relay to S

# M
W_EEms = (3e2 / fac) .* r_m # S to M
W_EEmd = (0 / fac) .* r_m # D to M
W_EImINs = (-3e2 / fac) .* r_m # INs to M
...
W_EIdINs = (-7.5e3 / fac) .* r_d # INs to D
W_EIdRet = (0 / fac) .* r_d # Reticular to D
W_EEdRel = (1e1 / fac) .* r_d # Relay to D
...
ZZ_normal = copy(Z)
Zx_normal = Z / maximum(Z)  # Normalização